...
# v6 修复 GitHub 仓库版本的 Bug

从 `FuriosoSlient/OILOJ` 拉取的版本无法正常工作，症状：

- `GET /problem/{id}` → **500** `NameError: name 'get_current_user' is not defined`
- 题库、题目、编辑页样式与功能全部错乱

## 根因

仓库里有一批**按另一套架构写的代码**被粘了进来，与本项目不兼容：

| 粘入的写法 | 本项目实际的写法 |
|---|---|
| `get_current_user(request)` | `user=Depends(current_user)` |
| `templates.TemplateResponse(...)`（Jinja2 服务端渲染） | `Render(...)` + 前端 JS 渲染 |
| `db.get_problem()` / `db.create_problem()` | `fetch_problem()` / `POST /api/admin/problem` |
| `user.get("role") in ("admin","problem_admin")` | `user["is_admin"]` + `contest_managers` 表 |
| 题目字段 `hint` | `constraints` / `background` / `sample_groups` |

具体造成四处问题：

1. **`/problem/{id}` 路由被整个替换**，引用了三个不存在的名字 → 必定 500。
2. **两个新路由写在 `uvicorn.run()` 之后**（`/problem/create`、`/problem/{id}/edit`），
   位于 `if __name__ == "__main__":` 块之下，**永远不会被注册**，属于死代码。
3. **`init_db()` 被整个替换**，只建 `problems` 一张表。原来的 12 张表
   （users/contests/submissions/hacks/...）和 `MIGRATIONS` 迁移机制全部丢失
   —— 现有库因为表已存在暂时没炸，但**换个新数据库会直接崩溃**。
4. **三个模板换成了 Jinja2 语法**（`{{ problem.title }}`、`{% if %}`），
   而本项目 `Render()` 只是读文件原样返回、由前端 JS 渲染，
   所以 `{{ }}` 会被当成字面文本显示出来。

## 修复

- 恢复 `/problem/{pid}` 为本项目写法（`Depends(current_user)` + `Render`）
- 删除 `uvicorn.run()` 之后的两个死路由，并加注释说明正确入口是 `POST /api/admin/problem`
- 恢复 `init_db()`：重新建全部 12 张表 + `MIGRATIONS` 增量迁移
- 删除 `db.py` 里 3 个基于错误 schema 的孤儿函数（`get_problems`/`create_problem`/`update_problem`）
- 恢复 `problem.html` / `problems.html` / `problem_edit.html` 为前端渲染版本

## 保留并正确实现「出题人」功能

那批代码想加的**出题人**功能是合理需求，我按本项目架构重新实现了：

- `author` / `author_id` 通过 `MIGRATIONS` 增量添加，不破坏既有数据
- `POST /api/admin/problem` 接收 `author` 字段
- **权限**：管理员可任意指定出题人；出题负责人只能署自己的名（后端强制覆盖，防伪造）
- `author_id` 只在**创建时**写入，后续编辑不会被改掉
- 展示：题目编辑页（负责人只读）、题面信息栏、题库列表新增「出题人」列

实测：管理员指定「张三老师」→ 改为「李四老师」，`author_id` 保持创建者不变；
出题负责人尝试署名「我想冒充管理员」→ 后端强制改回其本人「A3」。

## 验证结果

- 页面全部 200：`/` `/problems` `/problem/1` `/admin` `/admin/problem/1` `/contest/3` `/status` `/submission/63`
- 全新数据库 `init_db()` 正确建出 12 张表
- 评测回归：p-sum 正解 AC 100、编译错误 CE、SPJ 题另一组合法解 AC 100
- 真实 DOM 渲染六个页面，**零 JS 错误**
...
仓库体积 **71MB**，其中 66MB 是自动生成的测试数据（`data/problems/`），
每次 clone 都要拖一遍，既慢又没必要。

## 处理

### 删除的内容

- **14 道测试题**及其全部数据目录（p-sum / p-lis / p-graph / p-knapsack /
  p-string / t-xor / t-game / t-matrix / t-perm / x-mystery / demo-sq /
  spj-pair / mgr-demo / mgr-t2）
- 依赖这些题目的 25 条提交记录（避免留下悬空外键）
- `__pycache__/`、`data/bin/` 等缓存
...
|---|---|---|
| #15 | OIL OJ 入门教程 | 公开题 |
| #16 | **无归属者** | 按要求保留；5 个子任务、1 组样例、背景与题面完整 |

比赛 `OIL OJ Round #1` 及其对 #16 的引用一并保留。

> #16 在数据库里有子任务定义，但**数据文件本身仓库未上传**（`data/problems/uncredited/` 不存在），
> 需要在「管理后台 → 测试数据」里上传后才能评测。

**体积：71MB → 1.7MB。**

## `seed.py` 重写

原来的 `seed.py` 会 `DELETE FROM` 清空 10 张表、再重建 66MB 测试数据 ——
在现在这个库上运行只会**摧毁你的数据**。
...
python seed.py myname mypass      # 自定义管理员账号
```

- 只建表（含增量迁移）+ 创建**一个管理员账号**，不再有任何演示题目和假用户
- **幂等**：可重复运行；检测到已有管理员就直接跳过，绝不删除任何数据
...
```
data/problems/*        # 测试数据（保留 .gitkeep 占位）
data/oj.db             # 数据库含账号/提交/会话，不该入库
data/bin/  __pycache__/  *.exe
```

## 验证

- 页面全部 200，题库只剩应有的题目
- 全新库 `seed.py` 正确建出 12 张表、0 道题，二次运行安全
- 评测链路完好：新建题 → 上传数据 → 自动识别子任务 → 提交 **AC 100**

---

# 常见问题：解压覆盖后登不上账号

**原因**：`data/oj.db` 是数据库，账号、题目、提交记录全在里面。
之前的代码包为避免把账号信息传到 GitHub **特意排除了它**；
如果解压时替换掉整个 `oj/` 目录，数据库就一起没了 —— 表是空的，自然登不上。

## 恢复办法

**情况一：数据库还在（只是覆盖了代码）** → 直接能登。
若忘记密码、或账号不是管理员：

```bash
python seed.py --reset-admin <用户名> <新密码>
```

账号存在就重置密码并**提升为管理员**；不存在就新建一个管理员。
...
| 包 | 含 data/oj.db | 用途 |
|---|:--:|---|
| `oj_代码.zip` | 否 | 推送到 GitHub；数据库含账号密码，不应入库 |
| `oj_完整可运行.zip` | 是 | 本地直接跑，解压即用，保留现有账号与题目 |

> 升级时的正确做法：**只覆盖代码文件，不要动 `data/` 目录**，
> 保住 `data/oj.db` 和 `data/problems/` 即可。
...
`problem_visibility()` 只特判了 `is_admin`，**完全没有作者判定**。
非管理员的出题人建完题后，自己既在题库里看不到，打开也是 403「该题目未公开」——
等于刚写完的题目把自己锁在门外。

## 修复

新增 `is_author(user, problem)`：优先比对 `author_id`（建题时写入），
兼容早期没有该字段的数据时回退比对 `author` 名字。

在四个层面接入：

| 位置 | 改动 |
|---|---|
| `problem_visibility()` | 作者始终可见自己的题（含难度、Hack 数据） |
| `require_problem_owner()` | **新增**。单题的编辑权：管理员 / 负责人 / **本题作者** |
| `require_console_access()` | **新增**。出过题的人即可进后台（只看得到自己的题） |
| `/api/me` | 新增 `is_author`、`authored_count`，前端据此显示入口与「出题人」标记 |

改用 `require_problem_owner()` 的端点（6 个）：读取、删除、上传数据、
自动识别子任务、编译 SPJ、删除数据文件；`admin_save_problem` 在**编辑**时也走它。

### 一个副作用的处理

非管理员编辑他人题目时，原来的逻辑会把署名覆盖成自己。
现已改为**保留原作者署名**，只有管理员能改动出题人字段。

## 权限边界（实测）

纯出题人（`is_admin=False`、`is_manager=False`）：

| 操作 | 结果 |
|---|---|
| 题库看到自己的题 | ✅ |
| 打开 / 编辑自己的题、上传数据 | ✅ 200 |
| 进后台（只显示自己的题，无用户列表） | ✅ `role=author` |
| 读 / 删**别人的**题 | ❌ 403 |
| 改队伍分配、建队伍、任命负责人 | ❌ 403 |
| 完全没出过题的用户进后台 | ❌ 403 |

**比赛隔离未被削弱**：作者特权只对自己写的题生效。
实测比赛进行中，alice3 看别人的个人题仍是 403，
选手按位置可见的规则也完全不变。

## 后台界面

出题人登录后标题显示「我的题目」，只有「我的题目 / 测试数据 & SPJ」两个标签，
顶部提示其权限边界；侧栏与悬浮按钮出现管理入口，用户名旁标注绿色「出题人」。
